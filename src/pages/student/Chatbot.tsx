...
import { Loader2, Send, Plus, MessageCircle, Copy, Check, Trash2, Bot, User as UserIcon, Menu } from "lucide-react";
    ...
    <AppShell kind="student">
      ...
      <div className="grid h-[calc(100vh-12rem)] gap-4 md:grid-cols-[260px_1fr] md:h-[calc(100vh-6rem)]">
        ...
        <Card className="flex flex-col overflow-hidden">
          {/* Mobile chat header with conv-list trigger */}
          <div className="flex items-center justify-between border-b border-border p-2 md:hidden">
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" size="sm"><Menu className="mr-2 h-4 w-4" /> Chats</Button>
              </SheetTrigger>
              <SheetContent side="left" className="flex w-72 flex-col p-3">
                <Button onClick={newChat} className="mb-3 mt-6 gradient-primary text-primary-foreground"><Plus className="mr-2 h-4 w-4" /> New chat</Button>
                <ScrollArea className="flex-1">
                  {convs.length === 0 && <p className="px-2 py-6 text-center text-xs text-muted-foreground">No chats yet</p>}
                  <div className="space-y-1">
                    {convs.map((c) => (
                      <div key={c.id} className={`flex items-center justify-between rounded-md px-2 py-2 text-sm ${activeId === c.id ? "bg-primary/15 text-primary" : "hover:bg-muted"}`}>
                        <button onClick={() => setActiveId(c.id)} className="flex-1 truncate text-left">
                          <MessageCircle className="mr-2 inline h-3.5 w-3.5" />{c.title}
                        </button>
                        <button onClick={() => deleteChat(c.id)}><Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive" /></button>
                      </div>
                    ))}
                  </div>
                </ScrollArea>
